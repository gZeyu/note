...
## Approach 1
### 思路
可以使用动态规划求解该问题。令$s_{j,i}$表示范围[j, i)的子串，$dp[i]$表示$s_{0,i}$能否拆分成字典内的单词。如果能在[0,i )中找到分割点$j$，使得$s_{0,j}$和$s_{j,i}$，如果则递归方程如下:
$$
dp[i] = 
\begin{cases}
 true &if\; i=0\\
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjQ1Mzc0OTc1LC0yMDEyODcwMDcsMTE2NT
