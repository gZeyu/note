...
在搜索过程中维护一个$map$用于缓存搜索结果，$map$的$key$为字符串$s$的一个子字符串。该子字符串始于指定索引处的字符，一直到此字符串末尾，$value$存储着该子字符串能够拆分成字典内单词的所有情况。利用该$map$对搜索树剪枝，避免重复搜索。
### Java Code
``` Java
import java.util.*;  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTk2ODg3Nzk4MCwxMDIyMzg4NDEyXX0=
-->