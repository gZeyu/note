...
2
3.  维护一个哈希表，先遍历长字符串，将长字符串的每个字符作为键存入哈希表。接着遍历短字符串，若在哈希表中查询不到以短字符串的字符作为键的元素，则说明短字符串中的所有字符在长字符串中没有全部出现。
当长字符串长度远大于短字符串时，思路1的
### Java Code
``` Java
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTU4ODAwMzY4MiwxNDE4NjgyMjIyXX0=
-->