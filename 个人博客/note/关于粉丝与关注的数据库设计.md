...
### 查询两个用户关系状态
```sql
select sum(follower = user1_id and followed = user_2_id)+sum(follower = user_2_id and followed = user1_id) from blog_follower 
```
### 两个用户的共同关注列表
```sql
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTQ0MjU1NjE5NiwtMTg0OTc0NzcyNiwxOD
