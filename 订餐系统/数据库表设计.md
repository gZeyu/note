...
### 商店表
```
商店（id，用户id，商店名，管理者id，商店照片，商店信息，公告，评分）
```
```
o_shop(id, shop_name, manager_id, avatar, info, announcement, score)
```
> TODO ： 评分移到别的表？
### 外卖菜单
...
```
```
shipping_fee(id, shop_id, shipping_fee)
```
...
- 不可追评
```
订单评论（id，订单id，订购者id，父评论id，评论内容，评分，点赞数）
```
```
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzMDA1MDk5NzksMjEzNTc0NjgwMiwtMT
